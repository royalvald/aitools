...
仓库事实（可用性、LLM 画像）不再随 `bug_repo` 行逐 Bug 重复存储，而是集中在 `repo` 表（唯一键 path+branch）：同一仓库被 N 个 Bug 关联只登记/画像一次，画像跨 Bug 复用；`bug_repo` 退化为关联表（链接 + 本 Bug 维度的相关性判定，见 Spec 02 §9 v3）。
...
- **未声明仓库的 Bug**：不再直接拦下——登记表非空即放行，由 planning 的 `target_repos`（Spec 02 §9 v3）从候选登记表自动选仓；零选定才转 `repo_supplement` 介入。
...
| 未声明 + 登记表非空 | 放行 → 完整性评估 → planning 选仓 | 评估 1 次 + 方案生成 1 次（选仓并入方案输出；画像全局缓存命中时 0 次画像） |
...
| `AUTOBUGFIXER_REPO_PROFILE_ENABLED` | `true` | 全局画像 + 候选注入总开关（关闭时声明链接即绑定，下游回退基础仓库信息） |
| `AUTOBUGFIXER_REPO_CANDIDATE_LIMIT` | `20` | planning 候选仓库上限（声明链接 + 登记表补选） |
