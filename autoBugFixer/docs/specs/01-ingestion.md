...
| R6 | **（P1 待实现）**每个 Bug 接入时关联其修复涉及的全部本地仓库并逐个校验可用性；任一缺失/不可用 → 任务停 `WAIT_INFO` 并自动发起仓库补充询问，**不消耗 LLM 成本** | `task_ingest` 审计 `repo_check` + 介入单（§9） |
...
## 9. 修复关联仓库要求（P1 目标规格 · 待实现）

> 状态：**目标规格，当前未实现**。本节由 Spec 04 §8.6（评分代码实证不允许"无仓库降级"）派生。实现前的 as-built 行为见 §9.6 现状对照。
