...
> 状态：**设计定稿，未实现**（实现另行排期）。v1 照上述 as-built 运行。
...
- **触发**：AI 归类为 `cross_module`/`data_arch` 时，第二次 LLM 调用——输入 = bug 块 + 按标题/堆栈/关键词从**全部关联仓库**（Spec 01 §9）**只读检索**的相关文件片段（不建工作区，token 限量），产出实证结论（是否定位到疑似问题点、涉及文件数、改动面估计），驱动定位/波及维度
...
- **v1 打分无锚点/未校准**（驱动 §8 v2 重构）：模板三句话指示，尺度由模型自有知识生成，重跑可漂移，阈值建立在未校准尺度上
- `rationale` 允许为空且无质量校验
- 权重和不强制为 1（配置责任）
