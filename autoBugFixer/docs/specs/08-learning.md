...
流水线收口阶段：按最近一次验证结论走**双分支**——成功则把修复知识沉淀进经验库并关闭缺陷单，失败则用 LLM 复盘生成"不适用场景"并交给人工裁决。本阶段与 Spec 05 §4 的经验注入共同构成系统的**自我改进回路**（正向复用在本 spec §3.6 收口）。本 spec 以**双分支执行全过程时序**为主线（§2）。
