...
from autobugfixer.features.fixing.codex import (
    CodexError,
    CodexCLI,
    ScriptedCodexCLI,
    extract_usage,
...
from autobugfixer.common.core.models import FixRecord, LLMUsage
...
def _patch_run(monkeypatch, **kwargs):
    monkeypatch.setattr("autobugfixer.features.fixing.codex.subprocess.run",
...
def test_codex_missing_cli_raises(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("autobugfixer.features.fixing.codex.subprocess.run", not_found)
...
def test_codex_timeout_kills_attempt(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("autobugfixer.features.fixing.codex.subprocess.run", hang)
...
def test_codex_run_reads_last_message_and_usage(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("autobugfixer.features.fixing.codex.subprocess.run", fake_run)
...
def test_codex_preflight_reports_missing_cli_and_auth(monkeypatch, tmp_path):
    from autobugfixer.features.fixing.codex import codex_preflight

    monkeypatch.setattr("autobugfixer.features.fixing.codex.shutil.which", lambda _: None)
