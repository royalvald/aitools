...
from .ingestion import ingest_bug
...
class Scheduler:
    ...
    def dispatch_scored(self) -> list[int]:
        ...
        """
        from ..models import TaskStateHistory

        with self.session_factory() as s:
            tasks = list(s.scalars(select(Task).where(
                Task.state == TaskState.SCORED.value).order_by(
                Task.priority_score.asc()).limit(self.settings.scheduler_dispatch_limit)).all())
            task_ids = [t.id for t in tasks]
            for task in tasks:  # 出队迁移 + 留痕
                task.state = TaskState.FIXING.value
                task.current_stage = "scheduler"
                s.add(TaskStateHistory(
                    task_id=task.id, from_state=TaskState.SCORED.value,
                    to_state=TaskState.FIXING.value, stage="scheduler",
                    message="调度器按优先级出队"))
            s.commit()
    ...
    def _suspend_task(self, s: Session, it: Intervention, audit: AuditService) -> None:
        ...
        audit.log(action="task_suspended", target=f"task:{task.id}",
                  detail={"reason": f"介入单 #{it.id} SLA 超时"}, task_id=task.id)
