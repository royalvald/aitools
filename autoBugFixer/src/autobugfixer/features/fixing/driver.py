"""修复驱动工厂（Spec 05 扩展）：按 fix_driver 配置构建 codex / deepseek 通道。

两者同接口（run(prompt, workspace) -> CodexRunResult，失败抛 CodexError），
修复阶段不感知具体驱动；启动点预检按驱动分流（codex 查 CLI+鉴权，
...
def build_fix_driver(settings: Settings):
    """按 fix_driver 配置构建修复驱动（codex / deepseek）。"""
...
def fix_driver_preflight(settings: Settings) -> list[str]:
    ...
    errors: list[str] = []
    if settings.fix_driver != "deepseek":
        errors.append(f"fix_driver 非法: {settings.fix_driver!r}（可选 codex / deepseek）")
