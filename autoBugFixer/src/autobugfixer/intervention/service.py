...
from autobugfixer.core.models import BugTicket, Intervention, Task, TaskStateHistory, VerificationPlan
from autobugfixer.core.stage import InterventionRequest
    ...
    def resolve(self, intervention_id: int, result: dict, actor: str = "human") -> Task:
        ...
        if intervention.type == "info_supplement":
            ...
            for key, value in result.get("fields", {}).items():
                if hasattr(bug, key):
                    setattr(bug, key, value)
            from autobugfixer.ingest.repo_check import sync_bug_repos
            ...
            if result.get("approved", False):
                from autobugfixer.optimization.service import apply_strategy
